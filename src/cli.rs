...
pub struct Args {
    ...
    /// Safety level for payload filtering (1=aggressive, 5=safe read-only)
    #[arg(long, default_value_t = 3, value_parser = clap::value_parser!(u8).range(1..=5))]
    pub safety_level: u8,

    /// Output format (json, sarif, junit)
    ...
    /// Path to authentication macro YAML file
    #[arg(long)]
    pub auth_macro: Option<String>,

    /// Start in daemon mode (gRPC server)
    #[arg(long)]
    pub daemon: bool,

    /// Daemon listen address (default: 127.0.0.1:50051)
