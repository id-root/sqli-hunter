...
impl Scanner {
    ...
    pub fn with_safety_level(
        ...
        Ok(Scanner {
            ...
            latency_samples: Arc::new(Mutex::new(Vec::with_capacity(50))),
    ...
    async fn check_throttle(&self, current_latency: Duration) {
        // Update samples
        {
            let mut samples = self.latency_samples.lock().await;
            samples.push(current_latency);
            ...
            if samples.len() >= 10 {
                ...
                let mut baseline = self.baseline_latency.lock().await;
                *baseline = Some(avg);
            }
        }
        
        // Check if we need to throttle
        let baseline = self.baseline_latency.lock().await;
        if let Some(base) = *baseline {
            let threshold = base.mul_f64(self.throttle_multiplier);
            if current_latency > threshold {
                // Exponential backoff based on how much we exceeded
                let excess_ratio = current_latency.as_millis() as f64 / threshold.as_millis() as f64;
                let backoff_secs = (excess_ratio * 2.0).min(30.0);
                
                warn!(
                    "Latency spike detected ({:?} > {:?}). Backing off for {:.1}s",
                    current_latency, threshold, backoff_secs
                );
